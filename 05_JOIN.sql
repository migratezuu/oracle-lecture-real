...
-- 조인의 기본이 EQUAL JOIN이다. (EQU JOIN이라고도 함) (등가조인)
-- 일치하는 값이 없는 행은 조인에서 제외하는 것을 INNER JOIN 이라고 함. (NULL을 포함하지 않음)


-- JOIN의 기본은 INNER JOIN & EQUAL JOIN이다.
...
-- LEFT OUTER JOIN
-- ANSI 표준
SELECT  --NULL값을 가진 이효리 하동훈이 포함
       EMP_NAME
     , DEPT_TITLE
  FROM EMPLOYEE
--  LEFT OUTER JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); --OUTER빼도 결과가 같다.
--ORACLE 전용
SELECT
       EMP_NAME
     , DEPT_TITLE
...
--ORACLE 
-- 오라클 전용 구문으로는  FULL OUTER JOIN을 하지 못한다.!! 에러 발생
SELECT
       EMP_NAME
     , DEPT_TITLE
  FROM EMPLOYEE
...
-- NON EQUAL JOIC(NON EQU JOIN - 비등가조인) 
...
-- SELT JOIN : 동일한 테이블을 조인하는 것 (자가조인)
-- 오라클 전용
SELECT
       E1.EMP_ID
     , E1.EMP_NAME 사원이름
...
SELECT
     ...
     , LOCAL_NAME
  FROM EMPLOYEE
  JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) -- 조인 순서 유의
