...
4: HAVING 그룹을 식별하기 위한 조건
6: ORDER BY 컬럼명 | 별칭 | 컬럼순번 정렬방식([ASC]||[DESC]) [NULLS FIRST | NAST]
*/
...
-- 성별, 성별별 급여 평균(정수처리), 급여합계, 인원수를 조회한 뒤
-- 인원수로 내림차순 정렬하세요.
SELECT
...
-- 집계함수
-- ROLLUP 함수 : 그룹별로 중간 집계 처리를 하는 함수 GROUP BY 절에서만 사용한다.
SELECT
       JOB_CODE
     , SUM(SALARY)
  FROM EMPLOYEE
