...
SELECT   --MAINQUERY
       E.EMP_NAME
  FROM EMPLOYEE E
 WHERE E.DEPT_CODE = (SELECT E2.DEPT_CODE    --SUBQUERY (새로운 별칭 사용하는게 좋다.)
                      FROM EMPLOYEE E2
...
SELECT
       E.*
  FROM EMPLOYEE E
 WHERE EXISTS (SELECT E2.*              --200번 사원이 있어서 모든 값 출력(그냥 하나라도 있으면 출력)
               FROM EMPLOYEE E2
              WHERE E2.EMP_ID = '200'    
            );
...
-- 퇴사한 여직원과 같은 부서, 같은 직금에 해당하는 사원의 
-- 이름, 직급, 부서, 입사일 조회
SELECT
       E.EMP_NAME
     , E.ENT_DATE
...
SELECT
 ...
 WHERE V.순위 BETWEEN 10 AND 19; --중복된 수도 같이 출력함
...
--동일한 직급의 평균 급여보다 급여를 더 많이 받고 있는 직원의 정보 조회
SELECT
       E.EMP_NAME
     , E.JOB_CODE
     , E.SALARY
  FROM EMPLOYEE
 WHERE E.SALARY > (SELECT TRUNC(E2.SALARY), -5)
                     FROM EMPLOYEE E2
