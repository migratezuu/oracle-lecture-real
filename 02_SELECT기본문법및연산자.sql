...
SELECT
       EMP_NAME AS 이름
    ,  SALARY * 12 AS "1년 급여"
    , (SALARY + (SALARY + NVL(BONUS, 0))) * 12 "보너스를 포함한 연봉"  --NVL : NULL이 아니면 보너스를 아니면 0을 출력
  FROM EMPLOYEE;
...
WHERE SALARY > 5500000
    OR SALARY <3500000;   -- NOT BETWEEN연산과 동일한 값을 가짐.


  
  
-- LIKE 연산자 : 문자 패턴이 일치하는 값을 조회할 때 사용한다.
-- 컬럼명 LIKE '문자패턴'
-- 문자패턴 : '문자%' (문자로 시작하는 값)
--          '%문자%' (문자를 포함하는 값)
--          '%문자' (문자로 끝나는 값)
