...
SELECT
       LENGTH('오라클')
     , LENGTHB('오라클') --한글자에 3바이트 그래서 총 9
  FROM DUAL;            --DUMMY TABLE 가짜 테이블..
...
SELECT INSTR('AABAACAABA', 'B')FROM DUAL; --몇번째에 위치하는지 반환
SELECT INSTR('AABAACABAA', 'B', 1)FROM DUAL; --뒤에 1을 넣으면 뒤에서 부터
SELECT INSTR('AABAACAABAA', 'B', -1)FROM DUAL;
...
SELECT
       LPAD(EMAIL, 20, '#') --왼쪽 남는 칸에 #붙임
  FROM EMPLOYEE;

SELECT
       RPAD(EMAIL, 20, '#') --오른쪽 남는 칸에 #붙임
  FROM EMPLOYEE;


SELECT
       LPAD(EMAIL, 10) --10글자 이상은 잘라냄
  FROM EMPLOYEE;
...
SELECT LTRIM('   GREEDY', ' ')FROM DUAL;  --공백을 제거함
SELECT LTRIM('000123456', '0')FROM DUAL;
SELECT LTRIM('123123GREEDY', '123') FROM DUAL;
SELECT LTRIM('123123GREEYDT123','123')FROM DUAL;
SELECT LTRIM('ACABACCGREEDY', 'ABC')FROM DUAL; --한글자 한글자를 패턴으로 여김
SELECT LTRIM('5782GREEDY','0123456789')FROM DUAL

--SELECT 'GREEDY   ' FROM DUAL;
SELECT PTRIM('GREEDY   ',' ')FROM DUAL; 
...
SELECT TRIM('Z' FROM 'ZZGFGFGDGGZZZ')FROM DUAL;
SELECT TRIM(LEADING 'Z', FROM 'ZZZ123456ZZ') FROM DUAL;
SELECT TRIM(TRAILING 'Z' FROM 'ZZZ123456ZZ') FROM DUAL;
SELECT TRIM(BOTH '3' FROM '3333GEREEDY')FROM DUAL;

-- SUBSTR : 컬럼이나 문자열에서 지정한 위치보루터 지정한 갯수의 문자열을 잘라서 리턴
SELECT SUBSTR('SHOWMETHEMONEY', 5, 2)FROM DUAL; --5번째부터 2개 반환
...
-- 날짜 처리 함수 : SYSDATE, MONTHS_BETWEEN, ADD_MONTHS, NEXT_DAY, EXTRACT...
...
SELECT
       EMP_NAME
     , HIRE_DATE
     , LAST_DAY(HIRE_DATE) - HIRE_DATE + 1 입사월의근무일수 --1은 마지막날 입사한 사람도 있어서
  FROM EMPLOYEE;
...
SELECT
  ...
  FROM EMPLOYEE
--ORDER BY EMP_NAME ASC; --이름 오름차순, 내림은 DESC
--ORDER BY 사원이름 DESC; --별칭사용 가능, WHERE절에서는 사용 불가능
--ORDER BY 1;
...
SELECT TO_CHAR(1234, 'L9999')FROM DUAL;  --화폐기호를 붙여줌
...
SELECT TO_CHAR(SYSDATE, 'MON DY, YYYY') FROM DUAL; --DY는 수 DAY는 수요일
SELECT TO_CHAR(SYSDATE, 'YYYY-fmMM-DD DAY')FROM DUAL; --fmMM은 01월대신 1월로
...
--     , DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2','여')
     , DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남','여' )  --마지막 조건값은 생략 가능
  FROM EMPLOYEE;
